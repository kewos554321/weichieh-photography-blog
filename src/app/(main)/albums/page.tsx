...
import { Skeleton, AlbumGridSkeleton } from "@/components/skeletons/Skeleton";
        ...
        <div className="max-w-7xl mx-auto">
          {albums.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-[var(--text-muted)]">尚無相簿</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {albums.map((album) => (
                ...
                >
                  {/* Album Cover */}
                  <div className="relative aspect-[4/3] rounded-lg overflow-hidden bg-[var(--card-border)] mb-4">
                    {album.coverUrl ? (
                      <Image
                        src={album.coverUrl}
                        alt={album.name}
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <Images className="w-12 h-12 text-[var(--text-muted)]" />
                      </div>
                    )}
                    {/* Hover Overlay */}
                    <div className="absolute inset-0 bg-stone-900/0 group-hover:bg-stone-900/20 transition-colors duration-500" />
                    {/* Photo Count Badge */}
                    <div className="absolute bottom-3 right-3 px-2.5 py-1 bg-[var(--card-bg)]/90 backdrop-blur-sm rounded-full text-xs text-[var(--text-primary)] flex items-center gap-1">
                  ...
                  {/* Album Info */}
                  <h2 className="font-serif text-xl text-[var(--foreground)] group-hover:text-[var(--accent-teal)] transition-colors duration-300">
                    {album.name}
                  </h2>
                  {album.description && (
